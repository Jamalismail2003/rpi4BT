...
//extern lcapConnection myLcapLayer; // Assuming this is properly initialized elsewhere.
...
typedef struct rf_frame rf_frame;
...
typedef struct rf_command rf_command;
...
typedef struct dpnParameters dpnParameters;
...
typedef struct dpnCommand_t dpnCommand_t;
...
typedef struct rpnParameters rpnParameters;
...
typedef struct rpnCommand_t rpnCommand_t;
...
rfSession *findSession(rfcomm_context_t *rf_ctx, lcapConnection *lcn)
{
    ...
    {
        if (lcn == rf_ctx->m_sessions[i].lcn)
        {
            return &rf_ctx->m_sessions[i];
        }
    }
...
{
    assert(lcn);
    log_indented(8, "addSession(0x%04x)",lcn->local_cid);
    for (u16 i=0; i<RF_MAX_SESSIONS; i++)
    {
        if (!rf_ctx->m_sessions[i].lcn)
        {
            rfSession *session = &rf_ctx->m_sessions[i];
...
void deleteSession(rfSession *session)
{
    ...
    log_indented(8, "deleteSession(0x%04x)",session->lcn->local_cid);
    memset(session,0,sizeof(rfSession));
}
...
{
    assert(session && session->lcn);
    log_indented(8, "addChannel(0x%04x,%d)",session->lcn->local_cid,channel_num);
    for (u16 i=0; i<RF_MAX_CHANNELS; i++)
    {
        if (!session->channel[i].session)
        {
...
void deleteChannel(rfChannel *channel)
{
    ...
    log_indented(8, "deleteChannel(0x%04x,%d)",channel->session->lcn->local_cid,channel->channel_num);
    memset(channel,0,sizeof(rfChannel));
}
...
void sendPacket(rfChannel *channel, bool command, u8 frame_type, u8 *data, u16 data_len)
{
    ...
    *data_ptr++ = FCS((u8*)&cmd->frame, crc_len);
    u16 packet_len = ((uint32_t)((data_ptr) - (buf)));//((u32)data_ptr) - ((u32)buf);

    log_info("<--  RFComm - channel(%d) frame_type(0x%02x:%s) data_len(%d) packet_len(%d)",
        channel->channel_num, frame_type, frameTypeName(frame_type), data_len, packet_len);

    lcapLayer_sendData(buf, packet_len);   
}
...
static void lcapEvent(void *user_data, u16 event_type, lcapConnection *lcn,  const u8* buffer,  u16 length)
{
    ...
    rfcomm_context_t *rf_ctx = user_data;

    log_indented(8, "RFComm: event(%s) hci(0x%04x) cid(0x%04x) len=%d",
    ...
    switch (event_type)
    {
        ...
        {
            rfSession *session = findSession(rf_ctx, lcn);
            if (!session) return;
        ...
        case LCAP_EVENT_DATA :
        {
            ...
            rfEventHandler(rf_ctx, lcn, buffer, length);
            break;
        }
           
        default:
            log_error("RFComm: unhandled LCAP EVENT(%s) hci(0x%04x) cid(0x%04x) len=%d",
...
void rfEventHandler(rfcomm_context_t *rf_ctx, lcapConnection *lcn, const u8 *buffer, u16 length)
{
    ...
    printf("        RFComm: rfEventHandler(0x%02x:%s,%d) length=%d\n",
        frame_type, frameTypeName(frame_type), channel_num, length);
    ...
    // verify (that I understand) the FCS
        ...
        {
            printf("FCS matches fcs(%02x) calculated(%02x)",fcs,calculated);
        }
    #endif
    ...
    switch (frame_type)
    {
        case RF_FRAME_SABM :
        {
            log_indented(8, "RFComm: received SABM for channel(%d)", channel_num);
            // connection attempt from remote device
            // send the UA, and mark SABM and UA done
            log_info("<--  RFComm - sending UA for channel(%d)", channel_num);
        ...
        case RF_FRAME_UA  :
        {
            log_indented(8, "RFComm: received UA for channel(%d)", channel_num);
            channel->channel_state |= RF_CHANNEL_STATE_UA;
            break;
        }
        case RF_FRAME_UIH :
        {
            log_indented(8, "RFComm: received UIH for channel(%d)", channel_num);

            u8 len = p_frame->len >> 1;
            u8 *data = (u8*)&buffer[sizeof(rf_frame)];
            if (channel->channel_num)
            {
                // check if frame is long, meaning data len is two bytes, if yes, then
                // data pointer is pushed back by 1 byte and len is recalculated based on two bytes.
                if((u8)*(&p_frame->len + 1) == (u8)1) {
                    data = (u8*)&buffer[sizeof(rf_frame) + 1]; 
                    len = (*(&p_frame->len) | *(&p_frame->len + 1) << 8) >> 1;
                }

                //display_bytes(">UIH SPP",data,len);
                if (rf_ctx->m_pClient)
                {
                    log_indented(8, "RFComm: calling client rfCallback() data len:%d", length);
                    (*rf_ctx->m_pClient)(channel, data, len);
                    ...
                    log_error("RFComm: Invalid client rfCallback");
                }
            }
            else    //if to channel 0, it's a control frame ...
            {
                ...
                log_indented(8, "RFComm: DLC0 control type(0x%02x==>0x%02x:%s) %s len 0x%02x=>%d",
                    data[0],
                    c_type,
                ...
                switch (c_type)
                {
                    ...
                    default: 
                        log_error("RFComm: unsupported control type(0x%02x:%s) %s len=%d",
                            c_type, controlTypeName(c_type), c_command ? "command" : "response", c_len);
                        break;
                }
            }
            break;
        ...
        case RF_FRAME_DISC :
        {
            log_error("RFComm: unhandled frame type (0x%02x:%s) for channel(%d)", 
                frame_type, frameTypeName(frame_type), channel_num);
            break;
        }
    }
...
void processChannel(rfChannel *channel)
{
    ...
    if (!channel->channel_num)
    {
        if (!(channel->channel_state & RF_CHANNEL_STATE_INCOMING) &&
            !(channel->channel_state & RF_CHANNEL_STATE_SABM))
        {
            log_info("<--  RFComm - sending SABM for DLC0");
    ...
    {
        log_indented(8, "RFComm: processChannel(%d) state machine stopped for OPEN channel!",
            channel->channel_num);
        return;
    }
    ...
    {
        log_info("<--  RFComm - sending PN param negotiation for channel(%d)",
            channel->channel_num);
    ...
    else if (!(channel->channel_state & RF_CHANNEL_STATE_PN_RESPONSE_RECEIVED))
    {
        ...
        log_error("RFComm: Do nothing, we are waiting for a PN Response to our outgoing PN Command");
    }
             ...
             !(channel->channel_state & RF_CHANNEL_STATE_SABM))
    {
        ...
        log_indented(8, "RFComm: sending SABM packet for channel(%d)",
            channel->channel_num);
        sendPacket(channel,true,RF_FRAME_SABM,0,0);
        channel->channel_state |= RF_CHANNEL_STATE_SABM;
    }
    ...
    else if (!(channel->channel_state & RF_CHANNEL_STATE_SIGNALS_SENT))
    {
        ...
        printf("sending MSC signals for channel(%d)\n",
            channel->channel_num);
            sendSignals(channel);
    }
    ...
    {
        log_info("<--  RFComm - sending RPN param negotiation for channel(%d)",
            channel->channel_num);
    ...
    {
        log_indented(8, "CHANNEL(%d) opened!!! state=0x%04x",channel->channel_num,channel->channel_state);
...
void sendSignals(rfChannel *channel)
{
    ...
    log_info("<--  RFComm - Sending signals(%d)",channel->channel_num);
    rfChannel *dlc0 = findChannel(channel->session,0);
    if (!dlc0) return;
...
void controlModemStatus(rfChannel *dlc0, bool is_command, u8 *data, u8 len)
    ...
    //     it out ... we use shifted bitmasks instead ?!?! ...
{
    ...
    printf("MSC control for channel(0x%02x=%d) %s %s%s%s%s%s",
        data[0],
        channel_num,
    ...
    if (is_command) 
    {
        log_indented(8, "RFComm: Signals received(%d)",channel->channel_num);
    ...
    {
        log_indented(8, "RFComm: Signal response received(%d)",channel->channel_num);
...
void controlParamNegotiation(rfChannel *dlc0, bool is_command, u8 *data, u8 len)
    ...
    
{
    ...
    log_indented(8, "RFComm: controlParamNegotiation for DLCI(0x%02x)==channel(%d)",params->DLCI,channel_num);
    log_indented(8, "RFComm:    ft(%02x) cl(%02x) priorty(%02x) timer(%02x) mtu(%02x) retrans(%02x) recovery(%02x)",
        params->FrameType, params->ConvergenceLayer, params->Priority, params->ACKTimerTicks, params->MaximumFrameSize, 
        params->MaxRetransmissions, params->RecoveryWindowSize );
    ...
    if (is_command)
    {
        ...
        log_info("<--  RFComm - Sending param negotiation response(%d)",channel_num);
...
void controlRemotePortNegotiation(rfChannel *dlc0, bool is_command, u8 *data, u8 len)
    ...
    // set up the link’s parameters.
{
    ...
    log_indented(8, "RFComm: controlRemotePortNegotiation for DLCI(0x%02x)==channel(%d)",params->DLCI,channel_num);
    log_indented(8, "RFComm:    baud(%02x) dsp(%02x) flow(%02x) xon(%02x) xoff(%02x) mask(%04x)",
        params->baud_rate,
    ...
    if (is_command)
    {
        ...
        log_info("<--  RFComm - Sending remote param negotiation response(%d)",channel_num);
...
void rfLayer_sendData(rfChannel *channel, const u8 *data, u16 len)
{
    ...
    if (!channel->channel_num)
    {
        log_error("RFComm: illegal attempt to sendData() on dlc0");
        return;
    }
    if (!(channel->channel_state & RF_CHANNEL_STATE_OPEN))
    {
        log_error("RFComm: attempt to sendData() on unopened channel");
        return;
    }
    log_info("<--  RFComm - data len:%d", len);


display_bytes("Send to iPhone: ", data, len);
...
#if 0
        // FIXME
        lcapLayer_closeConnection(channel->session->lcn);
#endif
    ...
    // client must know, and pass in desired channel_num (from SDP)
{
    ...
    rfcomm_context_t *rf_ctx = (rfcomm_context_t *)lcapLayer_findClientByPSM(l2cap_ctx, RFCOMM_PSM);
    ...
    for (u16 i=0; i<RF_MAX_SESSIONS; i++)
    {
        rfSession *ses = &rf_ctx->m_sessions[i];
        if (ses->lcn &&
            ses->lcn->device &&
    ...
    if (!session)
    {
        lcapConnection *lcn = lcapLayer_startConnection(rf_ctx->l2cap_ctx, addr, RFCOMM_PSM);
    ...
    else
    {
        for (u16 i=0; i<RF_MAX_CHANNELS; i++)
        {
            if (session->channel[i].session &&
                session->channel[i].channel_num == channel_num)
            {
                log_error("RFComm: channel(%d) is already open in session(0x%04x)",
                    channel_num,
...
void rfcommLayer_destroy(void)
{
    // FIXME
#if 0
    myRfcommLayer->m_pClient = 0;
    myRfcommLayer->m_pClientThis = 0;    
...
{   
    log_info("RF: Registering client\n");
    
    rfcomm_context_t *rf_ctx = (rfcomm_context_t *)lcapLayer_findClientByPSM(l2cap_ctx, RFCOMM_PSM);

    rf_ctx->m_pClient = pClient;
}

int rfcomm_init(l2cap_context_t *l2cap_ctx)
{
    rfcomm_context_t *rf_ctx = (rfcomm_context_t *)malloc(sizeof(rfcomm_context_t));
    if(!rf_ctx) {
    ...
    rf_ctx->m_pClient = 0;
    rf_ctx->m_pClientThis = 0;    

    memset(rf_ctx->m_sessions, 0, RF_MAX_SESSIONS * sizeof(rfSession));

    rf_ctx->base.getPSM = getPSM;
    rf_ctx->base.serviceName = serviceName;
    rf_ctx->base.lcapEvent = lcapEvent;

    lcapLayer_registerClient(l2cap_ctx, (lcapClient*)rf_ctx);

    //client_init_queue();

    rf_ctx->l2cap_ctx = l2cap_ctx;
